...
(function($) {
  ...
  function setHeaderMenuSelection() {
    ...
    function setUnderline($item,idname) {
      if ($item && $item.length) {
        $item.addClass('active').siblings().removeClass('active');
      }
      if(!$('[id="'+idname+'"]').hasClass('menuitem')){
        $('.menuitem').each(function(){
          var id=$(this).attr('id')
          if(idname.indexOf(id)===0){
    ...
    if (idname && volantis.$headerMenu) {
      $active_link = $('[id="' + idname+'"]', volantis.$headerMenu);
      setUnderline($active_link,idname);
    }
  }
